...
export default ({ config, db }) => {
	...
	api.all('/login', async (req, res) => {
		...
		try {
			...
			let userinfo = await YuangongModel.findOne({ where: { yuangongzhanghao: req.query.username, mima: pwd } })
			if (userinfo === null) {
			...
			const token = jwt.sign(
				{
					...
					username: userinfo.dataValues.yuangongzhanghao,
					role: '员工',
                    loginUserColumn: 'yuangongzhanghao',
                    tableName: 'yuangong'
				},
				config.jwtSecret,
				{
			...
			userinfo.dataValues.role = '员工'
			userinfo.dataValues.loginUserColumn = 'yuangongzhanghao'
			userinfo.dataValues.tableName = 'yuangong'
	...
	api.post('/register', async (req, res) => {
		...
		try {
			...
			const userinfo = await YuangongModel.create(req.body)
	...
	api.all('/session', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await YuangongModel.findOne({ where: { id: req.session.userinfo === undefined ? jwt.decode(req.headers.token).id : req.session.userinfo.id } }))
		} catch(err) {
	...
	api.all('/resetPass', async (req, res) => {
		...
		try {
    		...
    		let pwd = '123456'
			await YuangongModel.update({
				mima: pwd
			}, {
				where: {
				  yuangongzhanghao: req.query.username
				}
			})
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let result = await YuangongModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await YuangongModel.findAll()
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			let yuangongzhanghaocount = await YuangongModel.count({
				where: {
					yuangongzhanghao: req.body["yuangongzhanghao"]
				}
			})
            if (yuangongzhanghaocount > 0) {
				toRes.session(res, -1, '员工账号已存在')
            }
		...
		try {
			let yuangongzhanghaocount = await YuangongModel.count({
				where :{
					...
					yuangongzhanghao:{
						[Op.eq]:req.body.yuangongzhanghao
					}
				}
			})
            if (yuangongzhanghaocount > 0) {
			...
			await YuangongModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await YuangongModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM yuangong WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM yuangong WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM yuangong WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM yuangong WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM yuangong WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM yuangong WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "yuangong"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM yuangong " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM yuangong " + where + " GROUP BY " + xColumnName + " DESC"
